...
std::vector<std::string>* VaccumCleaner::cleanHouse(House& h, CleaningAlgorithm& alg, int maxSteps) {
    ...
    std::vector<std::string> directionsTranslate{"North", "East", "South","West"};
    ...
    while (steps < maxSteps) {
        ...
        if (action == STAY) {
            ...
            std::string logMessage = "step: " + std::to_string(steps) + ". battery: " + std::to_string(getBatterySteps()) + ". current location: [" + std::to_string(std::get<0>(curLoc)) + "," + std::to_string(std::get<1>(curLoc)) + "]. action: move in direction " + directionsTranslate[action];
            //std::cout << logMessage << std::endl;
            stepLog->push_back(logMessage);
            continue;
        }
