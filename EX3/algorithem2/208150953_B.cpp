#include "208150953_B.hpp"

REGISTER_ALGORITHM (_208150953_B);

_208150953_B::_208150953_B() {
    currentLocation = {0,0};
    houseMap.markVisited(0, 0, 0);
}

void _208150953_B::setBatteryMeter(const BatteryMeter& meter) {
    batteryMeter = &meter;
    maxBattery = batteryMeter->getBatteryState();
    houseMap.setMaxBattery(maxBattery);
}

void _208150953_B::setWallsSensor(const WallsSensor& sensor) {
    wallsSensor = &sensor;
}

void _208150953_B::setDirtSensor(const DirtSensor& sensor) {
    dirtSensor = &sensor;
}

void _208150953_B::setMaxSteps(std::size_t max) {
    maxSteps = max;
}

pair<int,int> _208150953_B::calcNewPosition(Direction d, pair<int,int> pos) {
